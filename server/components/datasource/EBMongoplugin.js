...
class EBMongoPlugin extends EBDataSourcePlugin
{
    ...
    test(dataSource)
    {
        ...
        {
            mongodb.MongoClient.connect(dataSource.database.uri, function(connectError, db)
            {
                if (connectError)
                {
    ...
    lookupTables(dataSource)
    {
        return Promise.fromCallback(function(done)
        {
            ...
            mongodb.MongoClient.connect(database.uri, function(connectError, db)
            {
                if (connectError)
                {
                    return done(connectError);
    ...
    {
        return mongodb.MongoClient.connect(dataSource.database.uri).then((db) =>
        {
            // Get the desired collection
            const collection = db.collection(dataSource.database.collection);
            ...
            return queryObject.toArray().then((rows) =>
            {
                return db.close().then(() =>
                {
                    return rows.map((row) =>
                    {
                        const newRow = underscore.omit(row, "_id");
                        newRow.id = row._id;
                        return newRow;
                    });
                });
